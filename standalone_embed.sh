...
wait_for_milvus_running() {
    ...
    while true; do
	    ...
	    echo "Access the Milvus console at: http://127.0.0.1:9091/webui/"
...
start() {
    ...
    if [ "$res" -eq 1 ]; then
	podman pull docker.io/milvusdb/milvus:v2.5.12
        podman start milvus-standalone > /dev/null
    else
        run_embed
    fi
...
stop() {
    podman stop milvus-standalone > /dev/null
    if [ $? -ne 0 ]; then
        echo "Stop failed."
        exit 1
    fi
...
delete_container() {
    ...
    podman rm milvus-standalone > /dev/null
    if [ $? -ne 0 ]; then
...
upgrade() {
    read -p "Please confirm if you'd like to proceed with the upgrade. Confirm with 'y' for yes or 'n' for no. > " check
    if [ "$check" == "y" ] || [ "$check" == "Y" ]; then
        ...
        bash standalone_embed_latest.sh start > /dev/null && \
